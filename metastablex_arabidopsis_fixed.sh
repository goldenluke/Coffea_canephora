...
echo "🌱 MetastableX Arabidopsis (FIXED + PAPER READY)"
...
# DOWNLOAD
# =========================
wget -q -O data/arabidopsis.fa.gz https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/001/735/GCF_000001735.4_TAIR10.1/GCF_000001735.4_TAIR10.1_genomic.fna.gz
wget -q -O data/arabidopsis.gff.gz https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/001/735/GCF_000001735.4_TAIR10.1/GCF_000001735.4_TAIR10.1_genomic.gff.gz

gunzip -f data/arabidopsis.fa.gz
gunzip -f data/arabidopsis.gff.gz

# =========================
# PYTHON PIPELINE
# =========================
cat << 'PYEOF' > pipeline_fixed.py
import numpy as np
import pandas as pd
...
# =========================
# MERGE INTERVALS (FIX CRITICAL BUG)
# =========================
...
gene_fraction = gene_bases / len(seq)
...
# SAVE RESULTS
# =========================
df.to_csv("genomic_metastablex_fixed.csv", index=False)

print("\n💾 saved: genomic_metastablex_fixed.csv")
print("🚀 DONE")
PYEOF
